// Diff/Apply gegen die laufende lokale Supabase-DB (Service-Role aus .env).
//
// Befund #1 (Bericht): frühere Fassung testete gegen die echte Familie
// M2 G87 und hinterliess bei jedem Lauf Rückstände in der gemeinsam
// genutzten Katalog-DB (Admin-Feld short_text überschrieben und nie
// zurückgesetzt; deaktivierte Testzeilen, die beim "Wiederherstellen" per
// INSERT statt Reaktivierung neu angelegt wurden, sodass ein reales Produkt
// bei jedem Testlauf eine neue id bekam). Diese Fassung baut stattdessen
// eine eigene, synthetische Familie (Slug "test-diff-apply", siehe
// makeSyntheticFamily unten) auf, fasst keine echten Katalogdaten an und
// löscht die synthetische Familie am Ende vollständig (afterAll, läuft auch
// wenn ein it() fehlschlägt) - model_families -> models/products/
// product_fitment/pricelist_notes cascaded per on delete cascade
// (supabase/migrations/20260911000000_init.sql), ein einzelnes DELETE genügt.
//
// Ohne SUPABASE_SERVICE_ROLE_KEY/NEXT_PUBLIC_SUPABASE_URL in .env werden
// alle Tests hier sauber übersprungen (kein Fehlschlag in CI ohne DB).
//
...
try {
  ...
  // Datei fehlt oder Variablen sind bereits gesetzt (z. B. in CI) - dann
  // greift unten die reguläre hasSupabaseEnv()-Prüfung.
}
...
if (!hasSupabaseEnv()) {
  console.warn(
    "tests/pricelist/diff-apply.test.ts: NEXT_PUBLIC_SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY " +
      "fehlen in .env, überspringe die DB-Tests.",
  );
}
...
describe.skipIf(!hasSupabaseEnv())("diff.ts + apply.ts gegen die lokale DB (synthetische Testfamilie)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.from("model_families").delete().eq("slug", FAMILY_SLUG);
  });

  afterAll(async () => {
    if (!db) return;
    const { error } = await db.from("model_families").delete().eq("slug", FAMILY_SLUG);
    if (error) {
      // Nicht werfen: ein Aufräumfehler soll das Testergebnis nicht
      // überdecken, aber sichtbar sein.
      console.error(`Aufräumen der synthetischen Testfamilie fehlgeschlagen: ${error.message}`);
    }
  });

  it("importiert die Testfamilie initial: 2 Modelle, 4 Produkte, Fitment-Zeilen = Summe fits, 1 Hinweis", async () => {
    const result = await applyImport([family], db, {});
    ...
    const modelA = await db.from("models").select("id").eq("family_id", familyId).eq("slug", "modell-a").single();
    expect(modelA.error).toBeNull();
    modelAId = modelA.data!.id;

    // Admin-Feld setzen, um in den folgenden Schritten die Erhaltung zu prüfen.
    const setShortText = await db
      .from("model_families")
      .update({ short_text: TEST_SHORT_TEXT })
      .eq("id", familyId);
    expect(setShortText.error).toBeNull();
  });

  it("zweiter Import derselben Daten: Diff komplett unverändert, apply ändert nichts an Zeilenzahl/-menge", async () => {
    const diff = await buildDiff([family], db);
    ...
    expect(new Set(after.data!.map((r) => r.id))).toEqual(new Set(before.data!.map((r) => r.id)));

    const famRow = await db.from("model_families").select("short_text").eq("id", familyId).single();
    expect(famRow.data?.short_text).toBe(TEST_SHORT_TEXT);
  });
  ...
  it(
    ...
    async () => {
      ...
      const diff = await buildDiff([clone], db);
      const fd = diff.families[0];
      expect(fd.status).toBe("existing");
      ...
      const result = await applyImport([clone], db, {});
      expect(result.errors).toEqual([]);
      const fam = result.families[0];
      ...
      const removedRow = await db
        ...
        .select("active")
        .eq("family_id", familyId)
        .eq("name", "Zubehör ohne Fitment (Test)")
        .order("updated_at", { ascending: false })
        .limit(1)
        .maybeSingle();
      expect(removedRow.data?.active).toBe(false);

      const newRow = await db
        .from("products")
        .select("active, price_total")
        .eq("family_id", familyId)
        .eq("name", ADDED_PRODUCT_NAME)
        .eq("active", true)
        .maybeSingle();
      expect(newRow.data).not.toBeNull();
      ...
      const modelARowRes = await db.from("models").select("id").eq("family_id", familyId).eq("slug", "modell-a").single();
      const modelBRowRes = await db.from("models").select("id").eq("family_id", familyId).eq("slug", "modell-b").single();
      const fitmentARes = await db.from("product_fitment").select("product_id").eq("model_id", modelARowRes.data!.id);
      const fitmentBRes = await db.from("product_fitment").select("product_id").eq("model_id", modelBRowRes.data!.id);
      const productIdsForA = new Set((fitmentARes.data ?? []).map((r) => r.product_id));
      const productIdsForB = new Set((fitmentBRes.data ?? []).map((r) => r.product_id));

      const sistersRes = await db
        .from("products")
        .select("id, price_total")
        .eq("family_id", familyId)
        .eq("article_no", sisterA.articleNo!)
        .eq("active", true);
      expect(sistersRes.data?.length).toBe(2);
      const sisterARow = sistersRes.data!.find((r) => productIdsForA.has(r.id));
      const sisterBRow = sistersRes.data!.find((r) => productIdsForB.has(r.id));
  ...
  it("getProductsForModel liefert für Modell A nur dessen Fitments plus fits_all, Gruppen in Flow-Reihenfolge", async () => {
    const result = await getProductsForModel(modelAId, db);
    ...
    const fitmentRes = await db.from("product_fitment").select("product_id").eq("model_id", modelAId);
    const fittingIds = new Set((fitmentRes.data ?? []).map((r) => r.product_id));
    const allProductIds = result!.groups.flatMap((g) => g.products.map((p) => p.id));
    const productsRes = await db.from("products").select("id, fits_all").in("id", allProductIds);
    const fitsAllById = new Map((productsRes.data ?? []).map((p) => [p.id, p.fits_all]));
