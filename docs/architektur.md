...
- `body_styles text[]` (Karosserieformen, für die das Produkt gilt, leer = alle; `drive text` (xdrive | rwd | null); Entscheid 21.09.2026, Migration `db/migrations/0005_body_style_drive.sql`) - beide aus dem Namen abgeleitet, siehe `lib/catalog/body-style.ts` / `lib/catalog/drive.ts` und `docs/excel-import.md`, Abschnitt "Karosserieform und Antrieb"; wie `gearbox` bei jedem Import neu berechnet
...
- `body_style text` (nullable, Entscheid 21.09.2026, Migration 0005) - Karosserieform aus dem Fahrzeug-Schritt (Chips, nur wenn `CatalogModel.bodyStyleOptions` nicht leer) oder aus der Modellwahl `line` abgeleitet («Cabrio» → `cabrio`, `bodyStyleFromText()`); `drive text` (nullable, xdrive | rwd) - Antrieb aus dem Fahrzeug-Schritt (nur wenn `CatalogModel.driveOptions` nicht leer). `lib/inquiry/create.ts` prüft beide gegen die aktuellen Optionen des Modells, ungültige Werte werden `null`. Im Ticket, in der Inbox-Mail und in der Admin-Fahrzeugkarte als «Karosserie: Touring» / «Antrieb: xDrive», nur wenn gesetzt.
...
`outbound_emails`: `inquiry_id`, `type` (confirmation | inbox | reply | follow_up; die Check-Constraint erlaubt weiterhin auch `summary` für ältere, vor dem 17.09.2026 verschickte Zeilen - «Zusammenfassung an mich senden» wurde entfernt, siehe Abschnitt «Mail»), `to_email`, `subject`, `body_text`, `resend_id`, `status`, `error`, `sent_at`.
...
1. **Fahrzeug**: Marken-Chips (BMW, MINI, Toyota, Wiesmann). Familien-Kacheln mit Foto (Fallback: Kachel ohne Foto, Text «Foto folgt» nur im Admin, nicht im Flow). Nach Wahl der Familie: Motorisierungs-Chips (`models`), gruppiert nach `fuel`. Wenn `series_ps_suggested` mehrere Werte hat und `series_ps` nicht gesetzt ist: Chips «Serienleistung: 460 PS / 480 PS» zur Auswahl (filtert Leistungsprodukte nach `ps_base`). Ist die Baureihe für die gewählte Motorisierung mehrdeutig (`vehicleLineOptions()`, siehe Abschnitt „Fahrzeugbezeichnung“ Regel 6, z. B. X1/X2): zusätzlich Pflicht-Chips «Welches Modell fahren Sie?» (`inquiries.line`). Baujahr-Select, «Schon mal bei uns gewesen». Serien-PS/Nm anzeigen, wenn bekannt.
   ...
   - **Karosserie- und Antriebsfrage** (Entscheid 21.09.2026, `lib/catalog/body-style.ts`, `lib/catalog/drive.ts`, siehe `docs/excel-import.md` Abschnitt "Karosserieform und Antrieb"): Chips «Welche Karosserieform hat Ihr Fahrzeug?» (z. B. Limousine / Touring, Coupé / Cabrio, 3-Türer / 5-Türer / Cabrio) nur, wenn das Modell Produkte mit **unterschiedlichen** Karosserie-Zuordnungen fittet (`CatalogModel.bodyStyleOptions`) und die Modellwahl `line` die Karosserie nicht schon festlegt (4er G22/G23/G26: «Cabrio» reicht). Chips «Welchen Antrieb hat Ihr Fahrzeug?» (Heckantrieb / xDrive) nur, wenn Produkte für beide Antriebe fitten (`driveOptions`, im Bestand 5er F10/F11). Beide Pflicht vor «Weiter», kein «Weiss ich nicht». Im Kategorie-Schritt werden nicht passende Varianten ausgeblendet («... Touring» bei Limousine, «..., ohne xdrive» bei xDrive; `components/flow/state.ts` `effectiveBodyStyle()`/`bodyStyleSelectionVisible()`/`driveSelectionVisible()`), neutrale Produkte bleiben immer sichtbar; auch der «Passt gut dazu»-Vorschlag nimmt nur ein sichtbares Produkt. Eine spätere Änderung der Antwort entfernt bereits gewählte, nicht mehr passende Produkte (`SET_BODY_STYLE`/`SET_DRIVE`, wie `SET_GEARBOX`).
...
6. **Abschluss**: Nummer, Foto, Vorher/Nachher-Tabelle, Paket mit Richtsumme, «Paket als Link teilen» (kopiert `/p/<token>`, Web Share API auf Mobile), nächste Schritte. **Entfernt 17.09.2026** (Kundenrückmeldung Staging-Test): die Kachel «Zusammenfassung an mich senden» (rief `summary-mail`) - die Bestätigungsmail an den Kunden enthält dieselbe Zusammenfassung bereits, die Route/Vorlage (`buildSummary`) sind entfernt.
...
**6. Gewählte Alternative (Kundenentscheid 17.09.2026, «bei X1 und X2 gibt es dieselben Motorisierungen, das Modell ist X1 oder X2»):** statt der Baureihe eine eigene Excel-Zeile pro Modell zu geben, fragt der Flow bei einer mehrdeutigen Baureihe (Regel 1, Ausnahme oben) direkt «Welches Modell fahren Sie?» und speichert die Antwort als `inquiries.line` (Text-Id, z. B. `x2`, siehe `db/migrations/0004_inquiries_line.sql`). `vehicleLineOptions(family, model | null)` liefert die wählbaren Alternativen als `{ id, label, codes }` (leer, wenn die Familie für das Modell NICHT mehrdeutig ist oder ohne Modell) - `id` ist der Slug des Alternativ-Namens (`X2` → `x2`, `Grand Coupé` → `grand-coupe`, Diakritika entfernt), `codes` die eigenen Codes dieser Alternative (Regel 3). `vehicleDisplayLabel()` bekommt dafür einen optionalen vierten Parameter `lineId`: ist er gesetzt und unter den aktuellen Optionen gültig, wird GENAU diese Alternative mit ihren eigenen Codes verwendet (`BMW X2 20i (U10)`, `BMW 4er Cabrio 20i (G23)`) statt der Sammelform aus Regel 1 (`BMW X1 / X2 20i (U11, U10)`) - ein ungültiger/veralteter Wert (Baureihe seit dem letzten Import nicht mehr mehrdeutig, oder eine unbekannte Id) verhält sich wie kein `lineId` (Sammelform bleibt der sichere Rückfall). `vehicleInternalLine()` ergänzt bei gültigem `lineId` «· Modell: X2» an die interne Zeile. `lib/inquiry/create.ts` prüft den vom Client gesendeten Wert bei jeder Anfrage erneut gegen `vehicleLineOptions()` (nie ungeprüft übernehmen) und speichert bei einem ungültigen Wert `null` statt eines manipulierten Strings; der Prüfhinweis `modell_mehrdeutig` (siehe oben) feuert dadurch nur noch, wenn KEINE gültige Wahl vorliegt (Kundenflow erzwingt sie vor «Weiter», nur der Schnellweg ohne Angabe bzw. eine per Freitext nicht zuordenbare Extraktion lässt sie offen). Schnellweg: `lib/ai/extract.ts` liefert zusätzlich `vehicle.line` (frei erkannter Modellname, z. B. `"X2"`, kein Slug aus einer Liste), `lib/ai/to-payload.ts` gleicht ihn per Label-Vergleich gegen `vehicleLineOptions()` ab, `QuickInquiryForm.tsx` zeigt bei mehrdeutiger Familie zusätzlich ein Modell-Dropdown zum manuellen Nachtragen/Korrigieren. Ergänzung 21.09.2026 (Karosserieform/Antrieb): die Extraktion liefert ebenso `vehicle.body_style` und `vehicle.drive` (frei erkannt, z. B. «Touring», «xDrive»), `lib/ai/to-payload.ts` ordnet sie per `bodyStyleFromText()`/`driveFromText()` zu, das Formular zeigt Dropdowns «Karosserieform»/«Antrieb», sobald das Modell Optionen hat (`CatalogModel.bodyStyleOptions`/`driveOptions`).

## i18n

Eigene, leichte Lösung ohne Routing: `lib/i18n/de.ts` und `en.ts` exportieren dasselbe typisierte Objekt. `LocaleProvider` (Client) liest Cookie `lang` (de | en), Umschalter im Kopf des Flows. Serverseitig liest `layout.tsx` das Cookie und setzt `<html lang>`. Mails und Antwortentwurf verwenden `inquiries.locale`.

**Produkttexte auf Englisch (Entscheid 21.09.2026, Posten 4):** die Excel bleibt die einzige Quelle und ist deutsch. Damit ein englischsprachiger Kunde «Heckflügel Carbon» versteht, hält `product_translations` (`db/migrations/0006_product_translations.sql`) je Sprache eine Übersetzung PRO QUELLTEXT (nicht pro Produkt: derselbe Name kommt in vielen Baureihen vor, rund 950 verschiedene Texte für 2'700 Produkte). Schlüssel ist der normalisierte deutsche Text (`lib/translations/resolve.ts` `normalizeSourceText()`). Übersetzt werden Produktnamen, Beschreibungen (als Ganzes, mehrzeilig), bei Leistungsstufen nur die abgeleitete Restinformation (`productDisplay().detail`, z. B. «M6 & A8-Getriebe»; Titel «Stage 1» und PS/Nm sind schon lokalisiert), Gruppentitel, Excel-Kategorien und die Hinweistexte (`pricelist_notes`), siehe `lib/translations/texts.ts`. Gefüllt wird die Tabelle automatisch nach jedem Import (`app/admin/actions/pricelists.ts` per `after()`, `scripts/import-pricelists.ts` nach `--apply`) über `lib/translations/sync.ts` `translateMissing()`: nur fehlende Texte, in Batches per Claude Tool Use mit festem Fachglossar (`lib/translations/glossary.ts`, britisches Englisch), jede Übersetzung wird nachgeprüft (`lib/translations/ai.ts` `validateTranslation()`: alle Zahlen erhalten, gleiche Zeilenzahl, kein Gedankenstrich, dÄHLer erhalten), sonst verworfen und der Text bleibt deutsch. Ohne `ANTHROPIC_API_KEY` passiert nichts. Admin `/admin/uebersetzungen`: alle Katalogtexte mit Stand (automatisch / geprüft / fehlend), Suche, Korrektur je Zeile (origin `manual`, wird nie automatisch überschrieben; leer speichern löscht), Button «Fehlende übersetzen». Anzeige: `productDisplay()`/`displayItemFields()` (`lib/catalog/product-display.ts`) und `buildBeforeAfterRows()` nehmen optional eine `TranslationMap`; `/api/catalog/products` liefert die Map aller Texte eines Modells immer mit (`translations.en`), der Flow wendet sie bei `locale === "en"` an. Beim Anlegen einer Anfrage werden die Übersetzungen der gewählten Positionen in `inquiries.translations` eingefroren (wie `selections`), Bestätigungsmail, Teilen-Seite und Antwortentwurf einer englischen Anfrage lesen von dort. Interne Texte (Inbox-Mail, Admin, Prüfhinweise) bleiben deutsch. Alle Kundentexte, Kategorie-Texte, Folgefragen, Upsell-Texte, Prüfhinweise und Mailvorlagen in den Dictionaries, nie hart im JSX.
...
## Admin-Übersicht: Live-Aktualisierung und Zeilen (Stand 17.09.2026)

- `GET /api/admin/inquiries/heartbeat` (nur mit Session) liefert Gesamtzahl, jüngste `created_at` und Anzahl «neu». `components/admin/LiveRefresh.tsx` fragt alle 30 s (`ADMIN_POLL_MS`) nur bei sichtbarem Tab und sofort bei Rückkehr; bei Änderung `router.refresh()`, neue Zeilen 8 s hervorgehoben, Toast, Sidebar-Zähler zieht mit. Button «Aktualisieren» mit Zeitstempel. Kein Polling auf anderen Admin-Seiten.
- Die ganze Zeile (Desktop) bzw. Karte (Mobile) öffnet die Anfrage (Tastatur: Enter/Leertaste); die Nummer bleibt ein echter Link für «in neuem Tab öffnen».
