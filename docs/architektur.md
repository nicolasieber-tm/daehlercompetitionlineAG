...
## Fahrzeugbezeichnung (`lib/catalog/vehicle-label.ts`, Stand 15.09.2026, Feinschliff 15.09.2026)
...
**Bekannte Grenze (Ergänzung 15.09.2026, Feinschliff-Prüfung):** Baureihen, die mehrere Modelle unter denselben Motorisierungsnamen führen (`X1`/`X2`, `X3`/`X4`, `X5`/`X6` - je zwei Karosserieformen derselben Technik, gleiche Antriebe/Ausstattungslinien), kann die Formel nicht auflösen: teilt die Motorisierung mit keiner der beiden Alternativen ein Wort (z. B. eine reine Antriebsbezeichnung wie `20d`, die es für beide gibt), wählt Regel 1 stillschweigend die erste Alternative - ohne Grundlage, welches der beiden Modelle der Kunde tatsächlich hat. `lib/catalog/vehicle-label.ts` exportiert dafür `vehicleLineIsAmbiguous(family, model)`; `lib/rules/checks.ts` nutzt sie für den Prüfhinweis `modell_mehrdeutig` («Baureihe umfasst mehrere Modelle ..., Modell beim Kunden klären»), der dÄHLer in diesem Fall zur manuellen Klärung auffordert, statt die Mehrdeutigkeit stillschweigend zu verschlucken.
